...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_refund` AS (SELECT refund.trans_date, CONCAT( brand.brand_name, ' | ', item.item_desc, ' | ', item.item_add_desc) AS Description, category.category_name AS Category ,unit.unit_name AS Unit, 
refund_item_dtls.price, refund_item_dtls.qty, refund_item_dtls.sub_total, 
...
(SELECT refund.trans_date, service.service_desc AS Description, '' AS Category,'' AS Unit,refund_svc_dtls.price, refund_svc_dtls.qty, refund_svc_dtls.sub_total, 
CONCAT(C.user_gname, ' ', C.user_surname) AS Cashiers, CONCAT(M.user_gname, ' ', M.user_surname) AS Managers, refund.remarks, orders.branch_id FROM refund

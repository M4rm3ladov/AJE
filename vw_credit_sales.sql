...
credit_payment.invoice, CONCAT(brand.brand_name,' | ', item.item_desc,' | ', item.item_add_desc) AS DESCRIPTION, category.category_name AS Category, unit.unit_name AS Unit, order_item_dtls.price, order_item_dtls.qty, order_item_dtls.line_total, credit_payment.pay_amount,
...
(SELECT orders.trans_date AS DateInputed, credit_payment.trans_date AS transDate, CONCAT(customer_details.customer_gname, ' ', customer_details.customer_surname, ' ', customer_details.customer_suffix) AS Customers,
credit_payment.invoice, service.service_desc AS DESCRIPTION, '' AS Category, '' AS Unit, order_svc_dtls.price, order_svc_dtls.qty, order_svc_dtls.line_total, credit_payment.pay_amount,
CONCAT(user_details.user_gname,' ', user_details.user_surname,' ', user_details.user_suffix) AS Cashiers,  orders.branch_id FROM orders

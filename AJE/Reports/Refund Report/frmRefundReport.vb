...
Public Class frmRefundReport
    ...
    Private Sub btn_load_credit_Report_Click(sender As Object, e As EventArgs) Handles btn_load_credit_Report.Click
        ...
        Try
            With rv_credit_refund.LocalReport
                .ReportEmbeddedResource = "AJE.ReportCreditRefund.rdlc" '.ReportPath = Application.StartupPath & "\Report\ReportRefund.rdlc"
                .DataSources.Clear()
            End With
            Me.rv_credit_refund.RefreshReport()
            ...
            rv_credit_refund.LocalReport.SetParameters(rptParameter)

            ConnectDatabase()
            da.SelectCommand = New MySqlCommand("SELECT DATE_FORMAT(trans_date, '%m/%d/%Y') AS trans_date, invoice, Description, Category, Unit, price, qty, sub_total, Cashiers, Managers, remarks FROM vw_refund_credit WHERE trans_date BETWEEN '" & dtp_credit_From.Value.ToString("yyyy-MM-dd") & "' AND '" & dtp_credit_To.Value.ToString("yyyy-MM-dd") & "' AND branch_id = '" & frmMain.lbl_branch_Id.Text & "'", con)
            da.Fill(ds.Tables("dt_credit_Refund"))
            DisconnectDatabase()

            rptDs = New ReportDataSource("DataSet1", ds.Tables("dt_credit_Refund"))
            rv_credit_refund.LocalReport.DataSources.Add(rptDs)
            rv_credit_refund.SetDisplayMode(Microsoft.Reporting.WinForms.DisplayMode.PrintLayout)
            rv_credit_refund.ZoomMode = ZoomMode.PageWidth
        Catch ex As Exception
            DisconnectDatabase()
            MsgBox(ex.Message, vbCritical)
        End Try

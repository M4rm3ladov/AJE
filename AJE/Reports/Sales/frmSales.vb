...
Public Class frmSales
    ...
    Private Sub btn_load_cash_Report_Click_1(sender As Object, e As EventArgs) Handles btn_load_cash_Report.Click
        ...
        Try
            ...
            ConnectDatabase()
            da.SelectCommand = New MySqlCommand("SELECT DATE_FORMAT(trans_date, '%m/%d/%Y') AS trans_date, receipt, Description, price, qty, line_total, Cashiers FROM vw_cash_sales WHERE branch_id = '" & frmPos.lbl_branch_Id.Text & "' AND trans_date BETWEEN '" & dtp_cash_From.Value.ToString("yyyy-MM-dd") & "' AND '" & dtp_cash_To.Value.ToString("yyyy-MM-dd") & "'", con)
    ...
    Private Sub btn_credit_load_Report_Click(sender As Object, e As EventArgs) Handles btn_credit_load_Report.Click
        ...
        Try
            ...
            ConnectDatabase()
            da.SelectCommand = New MySqlCommand("SELECT DATE_FORMAT(trans_date, '%m/%d/%Y') AS trans_date, Customers, invoice, Description, price, qty, line_total, Cashiers FROM vw_credit_sales WHERE branch_id = '" & frmPos.lbl_branch_Id.Text & "' AND trans_date BETWEEN '" & dtp_credit_From.Value.ToString("yyyy-MM-dd") & "' AND '" & dtp_credit_To.Value.ToString("yyyy-MM-dd") & "'", con)

...
DROP VIEW IF EXISTS `vw_customer_transactions`$$

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_customer_transactions` AS 
(SELECT CONCAT(customer_details.customer_gname, ' ', customer_details.customer_surname, ' ',customer_details.customer_suffix) AS Customers, customer.customer_id,credit_payment.trans_date, credit_payment.pay_amount, credit_payment.invoice, CONCAT(brand.brand_name, ' | ', item.item_desc, ' | ', item.item_add_desc) AS Description, 
...
(SELECT CONCAT(customer_details.customer_gname, ' ', customer_details.customer_surname, ' ',customer_details.customer_suffix) AS Customers, customer.customer_id, credit_payment.pay_amount, credit_payment.trans_date, credit_payment.invoice, service.service_desc AS Description, 
